...
RUN apk update
RUN apk add --no-cache git make gcc g++ mariadb-dev mariadb-client-libs zlib-dev pcre-dev libressl-dev pcre libstdc++ nano dos2unix mysql-client bind-tools linux-headers bash
...
WORKDIR /emulator
ADD . /emulator/
RUN if [ ${PACKET_OBFUSCATION} == 1 ]; then sed -i "s|#define PACKET_OBFUSCATION|//#define PACKET_OBFUSCATION|g" /emulator/src/config/packets.hpp; fi \
    && if [ ${PACKET_OBFUSCATION} == 1 ]; then sed -i "s|#define PACKET_OBFUSCATION_WARN|//#define PACKET_OBFUSCATION_WARN|g" /emulator/src/config/packets.hpp; fi
...
CMD ["/emulator/athena-start", "watch"]